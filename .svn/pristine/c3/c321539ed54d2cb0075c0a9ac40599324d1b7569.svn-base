...
Public Class fmWebExportTest
    ...
    Private Sub btnExportNEW_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExportNEW.Click
        ...
        Try
            ...
            Dim dtItems As New DataTable
            Dim dtDimensions As New DataTable
            Dim dr As DataRow
            Dim drDim As DataRow
            Dim itms As String = Replace(Replace(tItems.Text, vbTab, ""), vbLf, ",")
            Dim itm As String
            Dim itmsIn As String = ""
            If Not pr02.Checked And Not rbcust.Checked Then
                ...
                'cmd.CommandText = "SELECT distinct itm_serial, itm_code, itm_desc, (case inactive when 1 then 0 else (SELECT MAX(itm_price_price) AS Expr1 FROM itm_price WHERE (itm_price_itm_code = itm_file.itm_code) AND (itm_price_type = 'RET') AND (itm_price_start <= GETDATE()) AND (itm_price_end >= GETDATE())) end) AS Price, (select MAX(itm_price_price) AS Expr1 from itm_price where (itm_price_itm_code = itm_file.itm_code) AND (itm_price_type = 'SHIP') AND (itm_price_start <= GETDATE()) AND (itm_price_end >= GETDATE()) and itm_price_qty_from<=1 and itm_price_qty_to>=1) AS Ship,1 as export FROM  itm_file, itm_price WHERE itm_code=itm_price_itm_code and itm_price_itm_code in (" + itmsIn + ") ORDER BY itm_code"
                cmd.CommandText = "SELECT distinct itm_serial, itm_code, itm_desc, (case inactive when 1 then 0 else (SELECT MAX(itm_price_price) AS Expr1 FROM itm_price WHERE (itm_price_itm_code = itm_file.itm_code) AND (itm_price_type = 'RET') AND (itm_price_start <= GETDATE()) AND (itm_price_end >= GETDATE())) end) AS Price, (select MAX(itm_price_price) AS Expr1 from itm_price where (itm_price_itm_code = itm_file.itm_code) AND (itm_price_type = 'SHIP') AND (itm_price_start <= GETDATE()) AND (itm_price_end >= GETDATE()) and itm_price_qty_from<=1 and itm_price_qty_to>=1) AS Ship,1 as export, itm_bottle_capacity FROM  itm_file, itm_price WHERE itm_code=itm_price_itm_code and itm_price_itm_code in (" + itmsIn + ") ORDER BY itm_code"
                da.Fill(dtItems)
            ...
            If Not pr02.Checked And Not rbcust.Checked Then
                ...
                For Each dr In dtItems.Rows
                    ...
                    'If dr("itm_bottle_capacity") = Nothing Then
                    '    bottlecapacity = 0
                    'Else
                    '    bottlecapacity = Convert.ToInt32(dr("itm_bottle_capacity"))
                    'End If
                    bottlecapacity = Convert.ToInt32(nz2(dr("itm_bottle_capacity"), 0))
                    Try
                        cnn0.Open()
                        ...
                        cmd0.Parameters.Add(New SqlParameter("@bottle_capacity", bottlecapacity))
                        If rbLIVE.Checked = True Then
                            ...
                            cmd0.CommandText = "update prod_tbl set prd_msrp=@msrp, prd_price=@price,prd_extra_ship_fee=@ship, prd_last_update=@now, prd_bottle_capacity = @bottle_capacity where prd_sku=@serial"
                        Else
                            ...
                            cmd0.CommandText = "update prod_tbl set prd_msrp_next=@msrp, prd_price_next=@price,prd_extra_ship_fee=@ship, prd_last_update=@now,prd_msrp_next_date=@priceDate, prd_price_next_date=@priceDate, prd_bottle_capacity = @bottle_capacity where prd_sku=@serial"
                        ...
                        cmd.CommandText = "select * from itm_box where itm_box_itm_code = '" + dr("itm_code") + "'"
                        da.Fill(dtDimensions)

                        If (Not (dtDimensions Is Nothing)) AndAlso (dtDimensions.Rows.Count > 0) Then

                            For Each drDim In dtDimensions.Rows

                                cmd0.Parameters.Clear()
                                cmd0.CommandText = "select count(*) from itm_box where itm_box_serial = " + Convert.ToString(drDim("itm_box_serial"))
                                Dim countDimension As Integer = cmd0.ExecuteScalar()

                                If countDimension > 0 Then
                                    cmd0.CommandText = "update itm_box set itm_box_len = " + Convert.ToString(drDim("itm_box_len")) + ", itm_box_width = " + Convert.ToString(drDim("itm_box_width")) + ", itm_box_height = " + Convert.ToString(drDim("itm_box_height")) + ", itm_box_weight = " + Convert.ToString(drDim("itm_box_weight")) + " where itm_box_serial = " + Convert.ToString(drDim("itm_box_serial"))
                                Else
                                    cmd0.CommandText = "insert into itm_box (itm_box_itm_code, itm_box_len, itm_box_width, itm_box_height, itm_box_weight, itm_box_name, itm_box_dept, itm_box_who, itm_box_when) values ('" + Convert.ToString(drDim("itm_box_itm_code")) + "', " + Convert.ToString(drDim("itm_box_len")) + ", " + Convert.ToString(drDim("itm_box_width")) + ", " + Convert.ToString(drDim("itm_box_height")) + ", " + Convert.ToString(drDim("itm_box_weight")) + ", '" + drDim("itm_box_name") + "', '" + drDim("itm_box_dept") + "', '" + drDim("itm_box_who") + "', '" + Convert.ToString(drDim("itm_box_when")) + "')"
                                End If
                                cmd0.ExecuteNonQuery()
                            Next
                    ...
                    If str2 <> "" Then
                        ...
                        If pr02.Checked Then
                            ...
                            Try
                                ...
                                cmd.Parameters.Clear()
                                cmd.CommandText = "select * where itm_box_itm_code = '" + dr("itm_code") + "'"
                                da.Fill(dtDimensions)
